...
\subsection{Purpose}
The purpose of this document is to provide a detailed description of the UnBox3D application. It will outline the system architecture, core components, user interfaces, and design considerations that support the conversion of complex 3D models into simplified geometric representations and corresponding 2D layouts for fabrication. This document serves as a foundation for understanding the design decisions and implementation strategies behind UnBox3D.
...
The UnBox3D application is a software tool that bridges the gap between digital modeling and physical assembly. It simplifies 3D models into basic geometric shapes and flattens them into printable 2D patterns. These layouts can then be used for rapid prototyping and fabrication. The application will provide a user-friendly interface and is designed to improve efficiency, accuracy, and accessibility in engineering and defense applications.

\section{External Interface Requirements}
\subsection{User Interface}
\begin{itemize}
    \item \textbf{Splash Screen:} The logo is shown as the program loads on the monitor
    \item \textbf{Import:} The viewport features an Import button where the user can either drop a file into it or click and pop up the user files to select what file they want to import into the program
    \item \textbf{Rendering Screen:} A screen where the 3d object is then rendered into a 2d image
    \item \textbf{Export:} on the bottom of the screen will be the export button for you to download
\end{itemize}
\subsection{Software Interfaces}
\begin{itemize}
    \item We will use OpenGL in order to get the import function working
